...
def complete_sub(num, num_set):
  return set(range(0, len(str(num)))) == num_set

def substring_10(num):
  str_num = str(num)
  if num in hash_num10:
    return complete_sub(num, hash_num10[num])
  ...
  if sum == 10:
    ...
    num_set.union(hash_num10[prenum])
    hash_num10[num] = num_set
    return complete_sub(num, num_set)
  else:
